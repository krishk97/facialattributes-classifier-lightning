...
class BaseDataset(Dataset):
    def __init__(self,
                 ...
                 transform=None,  # A function/transform that takes in a sample and returns a transformed version.
    ...
    def _get_raw_labels(self):
        if self._raw_labels is None:
            ...
            if self._raw_labels is None:
                self._raw_labels = torch.zeros([self._raw_shape[0], 0], dtype=torch.float32)
            assert isinstance(self._raw_labels, torch.Tensor)
            assert self._raw_labels.shape[0] == self._raw_shape[0]
            assert self._raw_labels.dtype in [torch.float32, torch.int64]
            if self._raw_labels.dtype == torch.int64:
                assert self._raw_labels.ndim == 1
                assert torch.all(self._raw_labels >= 0)
    ...
    def get_label(self, idx):
        ...
        if label.dtype == torch.int64:
            onehot = torch.zeros(self.label_shape, dtype=torch.float32)
            onehot[label] = 1
            label = onehot
        return label.copy()
    ...
    def label_shape(self):
        if self._label_shape is None:
            ...
            if raw_labels.dtype == torch.int64:
                self._label_shape = [int(torch.max(raw_labels)) + 1]
            else:
    ...
    def has_onehot_labels(self):
        return self._get_raw_labels().dtype == torch.int64
